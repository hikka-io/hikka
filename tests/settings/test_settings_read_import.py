...
async def test_settings_import_read(
    client,
    create_test_user,
    aggregator_manga,
    aggregator_novel,
    ...
    assert read.status == constants.READ_COMPLETED
    assert read.chapters == 60
    assert read.volumes == 20
    assert read.rereads == 0
    assert read.score == 9

    # Check log
    log = await test_session.scalar(select(Log).order_by(desc(Log.created)))
    assert log.log_type == constants.LOG_SETTINGS_IMPORT_READ
    assert log.data["imported_novel"] == 1
