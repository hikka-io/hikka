...
from client_requests import request_collection_random
from fastapi import status
from app import constants


async def test_collections_random(
    client,
    aggregator_anime,
    ...
    # Make sure we got correct response code
    assert response.status_code == status.HTTP_200_OK
    # Make request for random collection
    response = await request_collection_random(client)
    assert response.status_code == status.HTTP_200_OK
