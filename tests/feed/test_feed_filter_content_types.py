...
async def test_feed_filter_content_types(
    client,
    aggregator_anime,
    ...
    response = await request_feed(
        client, {"feed_content_types": ["collection"]}, get_test_token
    )
    ...
    # Only articles
    response = await request_feed(
        client, {"feed_content_types": ["article"]}, get_test_token
    )
    ...
    # Only comments
    response = await request_feed(
        client, {"feed_content_types": ["comment"]}, get_test_token
    )
    ...
    response = await request_feed(
        client,
        {"feed_content_types": ["article", "comment"]},
        get_test_token,
    )
