...
async def get_favourite(slug: str, user: User = Depends(auth_required)):
    # Find anime by slug
    ...
    if not (
        favourite := await AnimeFavourite.filter(
...
async def add_favourite(slug: str, user: User = Depends(auth_required)):
    # Find anime by slug
    if not (anime := await Anime.filter(slug=slug).first()):
        raise Abort("anime", "not-found")
    ...
    favourite = await AnimeFavourite.create(
        **{"created": datetime.utcnow(), "anime": anime, "user": user}
    )
...
async def delete_favourite(slug: str, user: User = Depends(auth_required)):
    # Find anime by slug
    if not (anime := await Anime.filter(slug=slug).first()):
        raise Abort("anime", "not-found")
